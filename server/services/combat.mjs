...
import { getExpForLevel, MAX_LEVEL } from '../shared.mjs';
import { getItem } from '../data/items.mjs';
...
export class CombatEngine {
  ...
  playerHeal(combat, player, healAmount) {
  ...
  usePotion(combat, player, itemId) {
    const inv = player.inventory.find(i => i.item.id === itemId);
    if (!inv || inv.quantity <= 0) return { combat, success: false, message: 'No potion available!' };

    const item = inv.item;
    if (item.type !== 'consumable' || !item.effect) return { combat, success: false, message: 'Not a usable item!' };
    ...
    if (effect.type === 'heal') {
      if (item.id.includes('mana')) {
        ...
        player.mp += healed;
        message = `${player.name} uses ${item.name}! Restored ${healed} MP.`;
      } else {
        const healed = Math.min(effect.value, player.maxHp - combat.playerHp);
        combat.playerHp += healed;
        player.hp = combat.playerHp;
        message = `${player.name} uses ${item.name}! Restored ${healed} HP.`;
      }
    }
    ...
    combat.log.push({
      turn: combat.log.length + 1, actor: 'player', action: 'heal',
      heal: effect.value, message, timestamp: Date.now(),
    });

    return { combat, success: true, message };
  }

  playerFlee(combat, player) {
  ...
  resolveCombat(combat, player, monster) {
    const drops = [];
